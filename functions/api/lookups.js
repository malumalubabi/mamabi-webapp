...
export async function onRequestGet({ env }) {
  try {
    ...
    const [customers, skus, suppliers, staff, paymentMethods, salesPlatforms] = await Promise.all([
      ...
      // registry_order (not alphabetical by sku) - the "01. SKU Registry"
      // sheet's row order, same as inventory/overview.js's Stock Overview
      // table - so every item dropdown across the app (New Order, Purchase
      // Log, Batch Production, Menu Engineering, Sales) reads in the same
      // order the business's own master list does, per explicit request.
      supabase.from("sku_items").select("id, sku, name, unit, item_type, category, min_stock, base_yield_qty, selling_price, platform_selling_price, status").eq("brand_id", brandId).order("registry_order"),
